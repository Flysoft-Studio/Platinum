...
let menuDownloadActive: HTMLElement, menuDownloadWaiting: HTMLElement, menuDownloadStopped: HTMLElement;
...
function getElements(gid: string) {
    ...
    if (!document.querySelector(prefix)) throw new Error("Download item " + gid + " not exists");
    return {
        ...
        progressInner: <HTMLElement>document.querySelector(prefix + " .download_progress>div"),
        speed: <HTMLElement>document.querySelector(prefix + " .download_speed"),
        openfolder: <HTMLLinkElement>document.querySelector(prefix + " .download_openfolder"),
        controls: {
            delete: <HTMLButtonElement>document.querySelector(prefix + " .download_btn_delete"),
            start: <HTMLButtonElement>document.querySelector(prefix + " .download_btn_start"),
            pause: <HTMLButtonElement>document.querySelector(prefix + " .download_btn_pause"),
            redownload: <HTMLButtonElement>document.querySelector(prefix + " .download_btn_redownload"),
            cancel: <HTMLButtonElement>document.querySelector(prefix + " .download_btn_cancel"),
            removeResult: <HTMLButtonElement>document.querySelector(prefix + " .download_btn_remove_result"),
        },
    }
}
...
function updateInfo(item: Download.Item, newItem: boolean = false) {
    ...
    if (newItem || fileNameMap[item.gid] != fileName) {
        ...
        if (item.status == "complete" && path && existsSync(path)) dlEle.icon.src = ipcRenderer.sendSync("get-icon", path);
        else dlEle.icon.src = ipcRenderer.sendSync("get-icon", extname(fileName));
        fileNameMap[item.gid] = fileName;
    }

    com.setElementVisible(dlEle.progress, item.status == "active" || item.status == "waiting" || item.status == "paused");
    com.setElementVisible(dlEle.controls.start, item.status == "paused");
    com.setElementVisible(dlEle.controls.pause, item.status == "waiting" || item.status == "active");
    com.setElementVisible(dlEle.controls.cancel, item.status == "waiting" || item.status == "active" || item.status == "paused");
    ...
    if (item.status == "active" || item.status == "waiting" || item.status == "paused") {
        ...
        dlEle.speed.innerHTML = lang.encode(((item.status == "active") ? (speedText + "/s, ") : ("")) + completedLengthText + " / " + totalLengthText);
    }
}
...
function updateDeleteStatus(item: Download.Item) {
    ...
    com.setElementVisible(dlEle.controls.redownload, deleted || item.status == "error" || item.status == "removed");
    com.setElementVisible(dlEle.controls.removeResult, finished || deleted || item.status == "error" || item.status == "removed");
    com.setElementVisible(dlEle.speed, deleted || item.status == "paused" || item.status == "removed" || item.status == "error" || item.status == "active" || item.status == "waiting" || item.status == "paused");
    if (!finished) dlEle.title.classList.add("link_disabled");
    else dlEle.title.classList.remove("link_disabled");
    if (item.status == "active" || item.status == "waiting" || item.status == "paused") dlEle.title.classList.add("link_disabled_noline");
...
function createItem(item: Download.Item, parent: HTMLElement) {
    ...
    let dlItem = <HTMLElement>document.querySelector(".download_item.template").cloneNode(true);
    ...
    if (path) {
        ...
        dlEle.controls.delete.onclick = () => {
            downloadMenu.menu.focus();
            if (existsSync(path)) {
                remote.shell.trashItem(path).catch((error) => {
                    log.error("Trash item failed, try using rm, path: " + path + ", reason: " + error);
                    try {
                        rmSync(path);
                        updateDeleteStatus(item);
                    } catch (error) {
                        log.error("Rm item failed, path: " + path + ", reason: " + error);
                    }
                }).then(() => {
                    updateDeleteStatus(item);
                });
    ...
    dlEle.controls.cancel.onclick = () => {
        ...
        if (path && existsSync(path)) {
            ...
            try {
                ...
                log.warn("Failed to remove aria2 file, path: " + path + ", reason: " + error);
            }
        }
    }
    ...
    dlEle.controls.redownload.onclick = () => {
        ...
        ipcRenderer.sendSync("download", item.files[0].uris[0].uri, getFilename(item.files[0]));
    }
    updateDeleteStatus(item);
...
function removeItem(item: Download.Item, parent?: HTMLElement) {
    ...
    let dlItem = <HTMLElement>document.querySelector(((parent) ? ("#" + parent.id + ">") : ("")) + "#download_" + item.gid);
    if (dlItem) dlItem.remove();
}

function updateBtnStatus() {
    // hides the download icon if there is nothing in the list
    com.setElementVisible(<HTMLElement>document.querySelector("#nav_download"), downloader.numActive != 0 || downloader.numWaiting != 0 || downloader.numStopped != 0);
}
...
export function showMenu() {
    ...
    if (sections.length != 0) downloadMenu.showMenuUnderElement("#nav_download", sections);
    else downloadMenu.hide();
}

function setTotalProgress() {
    let completedLength = 0, totalLength = 0, speed = 0;
    ...
    (<HTMLElement>document.querySelector("#nav_download_circle")).innerHTML = lang.encode(getSuitableUnit(speed).replace(" ", "\n") + "/s");
    (<HTMLElement>document.querySelector("#nav_download_circle")).style.clipPath = "polygon(0% 0%, " + percentage + " 0%, " + percentage + " 100%, 0% 100%)";
    (<HTMLElement>document.querySelector("#nav_download_icon")).style.clipPath = "polygon(" + percentage + " 0%, 100% 0%, 100% 100%, " + percentage + " 100%)";
...
window.addEventListener("load", () => {
    ...
    menuDownloadActive = <HTMLElement>document.querySelector("#menu_download_active"), menuDownloadWaiting = <HTMLElement>document.querySelector("#menu_download_waiting"), menuDownloadStopped = <HTMLElement>document.querySelector("#menu_download_stopped");
});
...
export function init(logger: ElectronLog) {
    ...
    ipcRenderer.on("download-status", (event, changedItems: Array<Download.ChangedItem>) => {
        ...
        for (let i = 0; i < changedItems.length; i++) {
            ...
            for (let i = 0; i < item.removed.length; i++) removeItem(item.removed[i], menu);
            for (let i = 0; i < item.added.length; i++) createItem(item.added[i], menu);
    ...
    ipcRenderer.on("download-active", () => {
        for (let i = 0; i < downloader.active.length; i++) updateInfo(downloader.active[i]);
        setTotalProgress();
    });
};