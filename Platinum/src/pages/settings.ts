...
function loadUser() {
    ...
    (<HTMLElement>document.querySelector("#set_user_main_name")).innerHTML = lang.encode(
        curUser.name ? curUser.name : lang.get("user_default")
    );
    com.setUserPicture("#set_user_main_picture", curUser.id, curUser.icon);
    com.setUserStatus(
        "#set_user_main_info",
        remote.getGlobal("syncStatus"),
        remote.getGlobal("syncError")
    );
    com.setElementVisible(
        <HTMLElement>document.querySelector("#set_user_desktoplink"),
        process.platform == "win32" && curUser.id != "default"
    );
}

function loadAbout() {
    (<HTMLElement>document.querySelector("#set_about_ver")).innerText = lang.get(
        "set_about_ver",
        [pkg.version, process.versions.chrome, process.arch]
    );
}

function loadAppearance() {
    com.setElementVisible(
        <HTMLElement>document.querySelector("#set_appearance_visual_blur"),
...
function loadPFM() {
    com.setElementVisible(
        <HTMLElement>document.querySelector("#set_pfm_sys_turbo"),
        !process.windowsStore
    );
    com.setElementVisible(
        <HTMLElement>document.querySelector("#set_pfm_sleep"),
        process.platform == "win32"
    );
}
...
function loadUpdate() {
    ...
    if (update && update.canUpdate) {
        ...
        let setStatus = (status: number) => {
            ...
            for (let i = 0; i < icons.length; i++) {
                let element = <HTMLElement>(
                    document.querySelector("#set_about_update_" + icons[i])
                );
                if (i == status) {
        ...
        if (update.status == "checking") {
            setStatus(1);
            text = lang.get("set_about_update_checking");
        } else if (update.status == "started") {
            setStatus(1);
            text = lang.get("set_about_update_downloading", [update.progress.toString()]);
        } else if (update.status == "waitinstall") {
            updateBtn = true;
            setStatus(2);
            text = lang.get("set_about_update_waiting");
        } else if (update.status == "error") {
            updateBtn = false;
            setStatus(2);
            text = lang.get("set_about_update_error", [update.error]);
        } else if (!update.available) {
            setStatus(0);
            text = lang.get("set_about_update_latest");
        } else if (update.available) {
            updateBtn = true;
            setStatus(0);
            text = lang.get("set_about_update_available", [update.version.str]);
        }
        (<HTMLElement>(
            document.querySelector("#set_about_update_btn_update")
        )).style.display = updateBtn ? null : "none";
        (<HTMLElement>document.querySelector("#set_about_update_text")).innerHTML =
            lang.encode(text);
    }
    com.setElementVisible(
        <HTMLElement>document.querySelector("#set_about_update"),
        update && update.canUpdate
    );
}
...
function loadInsider() {
    ...
    com.setElementVisible(
        <HTMLElement>document.querySelector("#set_about_update_btn_insider_join"),
        channel == "latest"
    );
    com.setElementVisible(
        <HTMLElement>document.querySelector("#set_about_update_btn_insider_exit"),
        channel == "preview"
    );
}
...
export function insiderAction() {
    ...
    if (!channel) channel = com.globalStore.get("update.channel") as string;
    if (channel == "preview")
        com.globalStore.set("applyrestart.update.channel", "latest");
    else if (channel == "latest")
        com.globalStore.set("applyrestart.update.channel", "preview");
