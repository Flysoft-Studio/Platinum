...
export function removeMultiRemove() {
    ...
    for (let i = 0; i < selectedItems.length; i++) {
        let historyItem = <HTMLElement>document.querySelector("#history_" + selectedItems[i]);
...
function updateRemoveMultiDlg() {
    ...
    (<HTMLElement>document.querySelector("#history_removemulti_title")).innerHTML = lang.encode(lang.get("his_removemulti_title", [selectedItems.length.toString()]));
}

function getElements(id: string) {
    const prefix = "#history_" + id;
    if (!document.querySelector(prefix)) throw new Error("History item " + id + " not exists");
    return {
        ...
        controls: {
            delete: <HTMLButtonElement>document.querySelector(prefix + " .history_btn_delete"),
        },
    }
}
...
ipcRenderer.on("history", (event, data) => {
    let historyItem = <HTMLElement>document.querySelector(".history_item.template").cloneNode(true);
    ...
    let isGeneralProtocol = data.url.startsWith("http://") || data.url.startsWith("https://") || data.url.startsWith("ftp://");
    ...
    historyEle.site.innerHTML = lang.encode((isGeneralProtocol) ? (urlStruct.hostname) : (data.url));
