...
export function reloadTColor() {
    ...
    if (
        ...
        verLte("6.2.0", osRelease())
    )
        tcolor = "#" + remote.systemPreferences.getAccentColor();
    ...
    addStyleSheet(
        tcolor ? "*,*::before,*::after{--theme-color:" + tcolor + ";}" : null,
        "com_style_tcolor"
    );
}
...
export function getImageColorInfo(pixels: Uint8Array) {
    ...
    var checkDark = () =>
        (isDark = rgba.r * 0.299 + rgba.g * 0.587 + rgba.b * 0.114 < 192);
...
export function registerEvents() {
    ...
    globalStore = new Store(
        normalize(remote.getGlobal("mgrDataDir") + "/config.json"),
...
export function setUserPicture(prefix: string, user: string, fileName: string) {
    ...
    setElementVisible(
        document.querySelector(prefix + " .user_default"),
        filePath == null
    );
    setElementVisible(
        document.querySelector(prefix + " .user_picture"),
        filePath != null
    );
    (<HTMLImageElement>document.querySelector(prefix + " .user_picture")).src = "";
    (<HTMLImageElement>document.querySelector(prefix + " .user_picture")).src = filePath
        ? filePath
        : "";
}
...
export function setUserStatus(
    prefix: string,
    status: Favourite.SyncStatus,
    errorMsg?: string
) {
    let icons = ["syncing", "idle", "error", "unset"];
    for (let i = 0; i < icons.length; i++) {
        let element = <HTMLElement>(
            document.querySelector(prefix + " .user_tips_" + icons[i])
        );
        if (status == "error" && icons[i] == status)
            (<HTMLElement>(
                document.querySelector(
                    prefix + " .user_tips_" + icons[i] + ">*:nth-child(2)"
                )
            )).innerHTML = lang.encode(lang.get("user_error", [errorMsg]));
