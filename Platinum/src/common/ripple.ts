...
export function registerRipple(element: HTMLElement) {
    ...
    element.addEventListener("touchstart", (event) => {
        if (event.target != element) return;
        let rect = element.getBoundingClientRect();
        element.style.setProperty("--ripple-x", (event.touches[0].clientX - rect.left).toString() + "px");
        element.style.setProperty("--ripple-y", (event.touches[0].clientY - rect.top).toString() + "px");
    });
    element.addEventListener("mousedown", () => {
        element.classList.add("ripple_active");
        setTimeout(() => requestAnimationFrame(() => {
            if (element) { element.classList.remove("ripple_active"); element.classList.remove("ripple_hover"); }
        }), 1000);
    });
}
