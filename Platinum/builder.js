...
const { dirname, resolve, extname, basename } = require("path");
...
if (process.platform == "linux")
    target = builder.Platform.LINUX.createTarget(undefined, builder.Arch.x64);
else if (process.platform == "win32")
    target = builder.Platform.WINDOWS.createTarget(undefined, builder.Arch.x64);
else if (process.platform == "darwin")
    target = builder.Platform.MAC.createTarget(
        undefined,
        builder.Arch.x64,
        builder.Arch.arm64
    );
            ...
            for (let i = 0; i < result.length; i++) {
                if (result[i].indexOf(".blockmap") == -1) {
                    ...
                    stream.on("end", () => {
                        ...
                        let exeName =
                            pkg.name + "_" + channelString + "_" + arch + extname(exe);
